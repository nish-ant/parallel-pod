...
#- Count number of snapshot directories before continuing
nSnap=$(find $outDIR -mindepth 1 -maxdepth 1 -type d | wc -l)
echo "Found ${nSnap} directories in ${outDIR}"
...
#- Tracking files
if [ $nSnap -gt 0 -a $cronEraseFlag -eq 1 ]; then
...
elif [ $nSnap -gt 0 -a $cronEraseFlag -eq 0 ]; then
    snapsDif=$outDIR/listDif.tmp
    ...
    cat $snapsDif | while read time || [[ -n $time ]]; do
        ...
        for xyPath in $outDIR/$time/*.xy; do
            ...
            if [ $updateCoordFlag -eq 1 ]; then
                ...
                if [ "$numLinesOld" -gt "$numLinesNew" ]; then
